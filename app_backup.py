...
style_choice = st.selectbox('Choose a style:', audio_analysis_descriptors)
if style_choice:
    # Show the distribution of activation values for the selected style.
    st.write(audio_analysis[style_choice].describe())

    style_range = st.slider(f'Select tracks with `{style_choice}` activations within range:', value=[0.5, 1.])

    # Filter the data to include only tracks that match the selected style and range.
    audio_analysis_query = audio_analysis[(pd.to_numeric(audio_analysis[style_choice], errors='coerce') >= style_range[0]) & (pd.to_numeric(audio_analysis[style_choice], errors='coerce') <= style_range[1])]
...
if st.button("RUN"):
    ...
    with open(m3u_filepaths_file, 'w') as f:
        ...
        mp3_paths = [os.path.join('..', str(mp3)) for mp3 in mp3s]
        f.write('\n'.join(mp3_paths))
